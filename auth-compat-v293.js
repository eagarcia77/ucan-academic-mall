...
const { patchBrowserXrEmulation, VERSION, BUILD } = require('./lib/browser-xr-emulation-v299');

const QUEST_RUNTIME_VERSION = VERSION;
const QUEST_RUNTIME_BUILD = BUILD;
const QUEST_RUNTIME_PATH = '/js/ucan_v299_quest_navigation_glass_terrace.js';
...
function transformCampusHtml(text) {
  ...
  html = html
    .replaceAll('Meta Quest V290:', 'Meta Quest V299:')
    .replaceAll('Meta Quest V293:', 'Meta Quest V299:')
    .replaceAll('Meta Quest V296:', 'Meta Quest V299:')
    .replaceAll('Meta Quest V298:', 'Meta Quest V299:')
    .replaceAll('V298: Meta Quest emula directamente la escena y el comportamiento del browser.', 'V299: navegación estable, cristales transparentes, escalera completa y terraza sólida en Meta Quest.')
    .replaceAll('V297: rótulos frontales legibles y velocidad del navegador con aceleración en Meta Quest.', 'V299: navegación estable, cristales transparentes, escalera completa y terraza sólida en Meta Quest.')
    .replaceAll('V296: movimiento igual a computadora, letreros de salas legibles y selección directa de planetas y pantallas en Meta Quest.', 'V299: navegación estable, cristales transparentes, escalera completa y terraza sólida en Meta Quest.');
...
function updateVersionData(data) {
  ...
  data.questArchitecture = 'single-browser-scene-quest-compatible';
  ...
  data.questJoystickAxisPairAutoDetection = true;
  data.questSynchronousSceneCollision = true;
  data.questAsynchronousCameraCollisionProxy = false;
  ...
  data.questRooftopStairRampCollisionDisabled = true;
  data.questRooftopStairDirectTraversal = true;
  data.questRooftopStairMidpointBlockPrevented = true;
  data.questGlassCompatibility = true;
  data.questGlassDepthPrePassDisabled = true;
  data.questGlassDepthWriteDisabled = true;
  data.questGlassAlphaBlend = true;
  data.questGlassBackFacesVisible = true;
  data.questRooftopCenterRemoved = true;
  data.questRooftopSolidFloor = true;
...
http.ServerResponse.prototype.writeHead = function writeHeadV299(statusCode, statusMessage, headers) {
  ...
  if (nextHeaders && typeof nextHeaders === 'object') {
    ...
    nextHeaders['X-UCAN-XR-Mode'] = 'browser-scene-quest-compatible';
    nextHeaders['X-UCAN-XR-Glass'] = QUEST_RUNTIME_VERSION;
...
http.ServerResponse.prototype.end = function endV299(chunk, encoding, callback) {
  let body = chunk;
  try {
    ...
    console.error('[UCAN V299 response compatibility]', error);
  }
...
console.info(`[UCAN ${QUEST_RUNTIME_VERSION}] Navegación, cristales y terraza Meta Quest cargados.`);
