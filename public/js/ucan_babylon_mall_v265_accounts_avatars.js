(() => {
  ...
  try {
    engine = new BABYLON.Engine(canvas, true, {
      preserveDrawingBuffer: true, stencil: true, antialias: true, powerPreference: 'high-performance'
    });
  } catch (primaryError) {
    if (loadStatus) loadStatus.textContent = 'Reintentando el motor 3D en modo compatible…';
    console.warn('[UCAN R37] WebGL de alto rendimiento no inició; se usará el modo compatible.', primaryError);
    try {
      engine = new BABYLON.Engine(canvas, true, {
        preserveDrawingBuffer: true, stencil: true, antialias: false,
        powerPreference: 'default', disableWebGL2Support: true
      });
      window.__UCAN_ENGINE_COMPATIBILITY_MODE_R37__ = true;
    } catch (compatibilityError) {
      const detail = String(compatibilityError?.message || primaryError?.message || 'WebGL no disponible');
      if (loadStatus) loadStatus.textContent = `No se pudo iniciar el entorno 3D: ${detail}`;
      if (loading) loading.dataset.ucanStartupError = 'engine';
      window.__UCAN_STARTUP_ERROR_R37__ = { stage:'engine', detail };
      console.error('[UCAN R37] No se pudo iniciar Babylon.js.', compatibilityError);
      return;
    }
  }
  ...
  function buildNaturalEnvironment(scene, root, mats, hemi, sunLight) {
    ...
    const api = {
      ...
      toggleCycle() {
        ...
        if (ENV_STATE.liveClock) ENV_STATE.timeOfDay = getSanGermanNow().timeOfDay;
        updateEnvironmentStatus();
        return ENV_STATE.liveClock;
      },
      useRealTime() {
        ENV_STATE.liveClock = true;
        ENV_STATE.cycleEnabled = false;
        ENV_STATE.timeOfDay = getSanGermanNow().timeOfDay;
        updateEnvironmentStatus();
        return { ...ENV_STATE };
      },
  ...
  function updateEnvironmentStatus() {
    ...
    if (cycle) {
      cycle.textContent = ENV_STATE.liveClock ? 'Hora real: activa' : (ENV_STATE.cycleEnabled ? 'Demostración día/noche: activa' : 'Iniciar demostración día/noche');
      cycle.title = ENV_STATE.liveClock ? 'El cielo coincide con la hora de Puerto Rico. Pulse para demostrar el ciclo acelerado.' : 'Pulse para volver a la hora real de Puerto Rico.';
      cycle.setAttribute('aria-pressed', ENV_STATE.liveClock ? 'false' : 'true');
    }
  }
