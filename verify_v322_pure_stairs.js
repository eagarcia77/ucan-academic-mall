...
const checks = {
  ...
  oneGroundResolutionPatch:/groundResolvedOncePerMovementFrame:true/.test(text.preloader) && text.preloader.includes('state.ground ya se resolvió una vez'),
  ...
  legacyBaseCallsRemovedAtResponse:text.preloader.includes('setupEscalatorRide(scene, camera);') && text.preloader.includes('__UCAN_LEGACY_ESCALATOR_RIDE_DISABLED_V323__') && text.preloader.includes('__UCAN_LEGACY_RELIABLE_MOVEMENT_DISABLED_V323__') && text.preloader.includes('__UCAN_LEGACY_CLAMP_HEIGHT_DISABLED_V323__'),
  ...
  authorityLoadedBeforeBase:text.preloader.includes('`${runtimeTag}\\n  ${authorityTag}\\n  $1`'),
  ...
  packageChecksV322:String(pkg.scripts?.check || '').includes('ucan_v322_stair_authority.js') && String(pkg.scripts?.check || '').includes('auth-compat-v323-browser-panel.js'),
...
const report = {
  version:'V322 preserved under V323',
  revision:'R27',
  feature:'Autoridad pura de altura de seis rutas preservada bajo el controlador de panel V323',
