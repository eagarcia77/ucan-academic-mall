...
// Base limpia: autenticación, persistencia, voz y presencia sin cargar las capas
// de locomoción anteriores. V319 controla la escena entregada al navegador.
require('./auth-compat-v313-parallel.js');

const VERSION = 'V319';
const REVISION = 'R23';
const BUILD = 'V319-20260730-SAFE-VR-LANDING-VISUAL-COMFORT-R23';
const RUNTIME_SRC = '/js/ucan_v316_complete_browser_vr_audit.js?build=V319-20260730-SINGLE-GROUND-LOCOMOTION-R23';
const LOADER_SRC = '/js/ucan_v319_social_loader.js?build=V319-20260730-SAFE-LANDING-VISUAL-COMFORT-LOADER-R23';
...
function transformHtml(value) {
  ...
  const runtimeTag = `<script src="${RUNTIME_SRC}" data-ucan-v319-locomotion="true"></script>`;
  ...
  const loaderTag = `<script src="${LOADER_SRC}" data-ucan-v319-social-loader="true"></script>`;
  if (renderParityPattern.test(html)) html = html.replace(renderParityPattern, `${loaderTag}\n  $1`);
  else html = html.replace('</body>', `  ${loaderTag}\n</body>`);

  html = html.replace(/UCAN Academic Mall V(?:272|283|313|314|315|316|317|318)/g, 'UCAN Academic Mall V319');
  html = html.replace(/COMPILACIÓN V(?:272|283|313|314|315|316|317|318)(?: · [^<]+)?(?: ACTIVA)?/g, 'COMPILACIÓN V319 · SALIDA SEGURA Y CONFORT VISUAL');
  html = html.replace(/V(?:272|283|313|314|315|316|317|318):[^<]*/g, 'V319: salida segura al Piso 2, controlador único de altura y brillo cómodo en todos los entornos.');
  html = html.replace('</head>', `  <meta name="ucan-runtime-v319" content="${BUILD}" />\n</head>`);
  return html;
}
...
function patchRenderParity(source) {
  let code = String(source || '');
  const comfort = `function comfortV319(requested){let z=0,s=st.scene;if(!s)return z;let mode=requested;try{mode=mode||localStorage.getItem('ucanV319VisualMode')||'comfort'}catch(_){mode=mode||'comfort'}if(!['comfort','dim','normal'].includes(mode))mode='comfort';s.metadata={...(s.metadata||{})};if(!s.metadata.ucanV319BaseVisual){const i=s.imageProcessingConfiguration;s.metadata.ucanV319BaseVisual={exposure:n(i?.exposure??1),contrast:n(i?.contrast??1),environmentIntensity:n(s.environmentIntensity??1)}}const base=s.metadata.ucanV319BaseVisual,p=mode==='dim'?{exposure:.58,contrast:1.02,environment:.52,light:.60}:mode==='normal'?{exposure:base.exposure,contrast:base.contrast,environment:base.environmentIntensity,light:1}:{exposure:.72,contrast:1.04,environment:.65,light:.74};const i=s.imageProcessingConfiguration;if(i){if(!eq(i.exposure,p.exposure,.001)){i.exposure=p.exposure;z++}if(!eq(i.contrast,p.contrast,.001)){i.contrast=p.contrast;z++}if('toneMappingEnabled'in i&&!i.toneMappingEnabled){i.toneMappingEnabled=true;z++}}if(!eq(s.environmentIntensity,p.environment,.001)){s.environmentIntensity=p.environment;z++}for(const l of s.lights||[]){l.metadata={...(l.metadata||{})};if(!Number.isFinite(Number(l.metadata.ucanV319BaseIntensity)))l.metadata.ucanV319BaseIntensity=n(l.intensity);const target=n(l.metadata.ucanV319BaseIntensity)*p.light;if(!eq(l.intensity,target,.001)){l.intensity=target;z++}}window.__UCAN_VISUAL_COMFORT_V319__={version:'V319',revision:'R23',mode,exposure:p.exposure,contrast:p.contrast,environmentIntensity:p.environment,lightFactor:p.light,sameBrowserVr:true};return z}\nwindow.__UCAN_APPLY_VISUAL_COMFORT_V319__=comfortV319;\n`;
  code = code.replace(
    'function profile(){if(!st.scene||!st.engine)return 0;let z=0;',
    `${comfort}function profile(){if(!st.scene||!st.engine)return 0;let z=comfortV319();`
  );
  code = code.replace('z+=rs();st.repairs++;', 'z+=rs();z+=comfortV319();st.repairs++;');
  return code;
}
...
function transformJson(value) {
  try {
    ...
    return JSON.stringify({
      ...
      revision:REVISION,
      build:BUILD,
      architecture:'single-ground-controller-safe-landing-visual-comfort',
      ...
      visualComfortDefault:'comfort',
      visualComfortExposure:0.72,
      visualComfortEnvironmentIntensity:0.65,
      visualComfortSameBrowserVr:true,
      visualComfortUserSelectable:true,
...
http.ServerResponse.prototype.writeHead = function writeHeadV319(statusCode, statusMessage, headers) {
  ...
  if (transformable) this.__ucanV319Chunks = [];
  try {
    this.removeHeader?.('Content-Length');
    this.setHeader?.('X-UCAN-Version', VERSION);
    this.setHeader?.('X-UCAN-Revision', REVISION);
    this.setHeader?.('X-UCAN-Comfort', 'safe-landing-visual-comfort');
    if (transformable) {
  ...
  if (nextHeaders && typeof nextHeaders === 'object') {
    ...
    nextHeaders['X-UCAN-Comfort'] = 'safe-landing-visual-comfort';
    if (transformable) {
...
http.ServerResponse.prototype.write = function writeV319(chunk, encoding, callback) {
  if (Array.isArray(this.__ucanV319Chunks)) {
    if (chunk != null) this.__ucanV319Chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk), typeof encoding === 'string' ? encoding : 'utf8'));
...
http.ServerResponse.prototype.end = function endV319(chunk, encoding, callback) {
  let body = chunk;
  try {
    if (Array.isArray(this.__ucanV319Chunks)) {
      if (body != null) this.__ucanV319Chunks.push(Buffer.isBuffer(body) ? body : Buffer.from(String(body), typeof encoding === 'string' ? encoding : 'utf8'));
      const combined = Buffer.concat(this.__ucanV319Chunks).toString('utf8');
      delete this.__ucanV319Chunks;
      const pathname = requestPath(this);
    ...
    console.error('[UCAN V319 response]', reason);
  }
  return baseEnd.call(this, body, encoding, callback);
};

console.info(`[UCAN ${VERSION} ${REVISION}] Salida segura y confort visual activos (${BUILD}).`);
