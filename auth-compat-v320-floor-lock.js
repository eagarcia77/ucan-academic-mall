...
const RUNTIME_SRC = '/js/ucan_v316_complete_browser_vr_audit.js?build=V320-20260730-SINGLE-GROUND-LOCOMOTION-R24';
const LOADER_SRC = '/js/ucan_v320_social_loader.js?build=V320-20260730-FLOOR-TWO-STICKY-LOCK-LOADER-R24';
...
function transformHtml(value) {
  ...
  for (const script of [
    ...
    'ucan_v318_floor_route_controller.js',
    'ucan_v319_social_loader.js',
    'ucan_v319_floor_route_controller.js',
    'ucan_v319_vr_accessibility.js',
    'ucan_v320_social_loader.js',
    'ucan_v320_floor_lock_controller.js'
  ...
  const runtimeTag = `<script src="${RUNTIME_SRC}" data-ucan-v320-locomotion="true"></script>`;
  ...
  const loaderTag = `<script src="${LOADER_SRC}" data-ucan-v320-social-loader="true"></script>`;
  if (renderParityPattern.test(html)) html = html.replace(renderParityPattern, `${loaderTag}\n  $1`);
  else html = html.replace('</body>', `  ${loaderTag}\n</body>`);

  html = html.replace(/UCAN Academic Mall V(?:272|283|313|314|315|316|317|318|319)/g, 'UCAN Academic Mall V320');
  html = html.replace(/COMPILACIÓN V(?:272|283|313|314|315|316|317|318|319)(?: · [^<]+)?(?: ACTIVA)?/g, 'COMPILACIÓN V320 · PISO 2 ESTABLE');
  html = html.replace(/V(?:272|283|313|314|315|316|317|318|319):[^<]*/g, 'V320: el Piso 2 permanece estable hasta una bajada intencional.');
  html = html.replace('</head>', `  <meta name="ucan-runtime-v320" content="${BUILD}" />\n</head>`);
  return html;
}
...
function patchLocomotion(source) {
  ...
  code = code.replace(
    ...
    `function groundFor(position) {\n    const sharedGround = window.__UCAN_FLOOR_ROUTE_CONTROLLER_V320__?.resolveGround?.(position, state.ground);\n    if (Number.isFinite(Number(sharedGround))) return Number(sharedGround);\n    const ramp = rampGround(position, state.ground);`
  );

  code = code.replace(
    'state.floor = state.ground;\n      state.velocity?.set?.(0, 0, 0);',
    `state.floor = state.ground;\n      window.__UCAN_FLOOR_ROUTE_CONTROLLER_V320__?.setFloor?.(state.ground, 'panel-navigation');\n      state.velocity?.set?.(0, 0, 0);`
  );
  code = code.replace(
    'state.floor = nearestFloor(state.ground);\n    camera.position.y = state.ground + currentEyeHeight();',
    `state.floor = nearestFloor(state.ground);\n    window.__UCAN_FLOOR_ROUTE_CONTROLLER_V320__?.setFloor?.(state.floor, 'teleport');\n    camera.position.y = state.ground + currentEyeHeight();`
  );
  code = code.replace(
    'state.floor = floor;\n    state.velocity.set(0, 0, 0);',
    `state.floor = floor;\n    window.__UCAN_FLOOR_ROUTE_CONTROLLER_V320__?.setFloor?.(floor, 'reset');\n    state.velocity.set(0, 0, 0);`
  );
  code = code.replace(
    "console.info('[UCAN V316 R20] Auditoría completa, locomoción por rig y panel unificado instalados.');",
    "console.info('[UCAN V320 R24] Locomoción con Piso 2 estable instalada.');"
  );
  return code;
}
...
function patchRenderParity(source) {
  let code = String(source || '');
  const comfort = `function comfortV320(requested){let z=0,s=st.scene;if(!s)return z;let mode=requested;try{mode=mode||localStorage.getItem('ucanV319VisualMode')||'comfort'}catch(_){mode=mode||'comfort'}if(!['comfort','dim','normal'].includes(mode))mode='comfort';s.metadata={...(s.metadata||{})};if(!s.metadata.ucanV320BaseVisual){const i=s.imageProcessingConfiguration;s.metadata.ucanV320BaseVisual={exposure:n(i?.exposure??1),contrast:n(i?.contrast??1),environmentIntensity:n(s.environmentIntensity??1)}}const base=s.metadata.ucanV320BaseVisual,p=mode==='dim'?{exposure:.58,contrast:1.02,environment:.52,light:.60}:mode==='normal'?{exposure:base.exposure,contrast:base.contrast,environment:base.environmentIntensity,light:1}:{exposure:.72,contrast:1.04,environment:.65,light:.74};const i=s.imageProcessingConfiguration;if(i){if(!eq(i.exposure,p.exposure,.001)){i.exposure=p.exposure;z++}if(!eq(i.contrast,p.contrast,.001)){i.contrast=p.contrast;z++}if('toneMappingEnabled'in i&&!i.toneMappingEnabled){i.toneMappingEnabled=true;z++}}if(!eq(s.environmentIntensity,p.environment,.001)){s.environmentIntensity=p.environment;z++}for(const l of s.lights||[]){l.metadata={...(l.metadata||{})};if(!Number.isFinite(Number(l.metadata.ucanV320BaseIntensity)))l.metadata.ucanV320BaseIntensity=n(l.intensity);const target=n(l.metadata.ucanV320BaseIntensity)*p.light;if(!eq(l.intensity,target,.001)){l.intensity=target;z++}}window.__UCAN_VISUAL_COMFORT_V319__={version:'V320',revision:'R24',mode,exposure:p.exposure,contrast:p.contrast,environmentIntensity:p.environment,lightFactor:p.light,sameBrowserVr:true};return z}\nwindow.__UCAN_APPLY_VISUAL_COMFORT_V319__=comfortV320;\n`;
  code = code.replace('function profile(){if(!st.scene||!st.engine)return 0;let z=0;', `${comfort}function profile(){if(!st.scene||!st.engine)return 0;let z=comfortV320();`);
  code = code.replace('z+=rs();st.repairs++;', 'z+=rs();z+=comfortV320();st.repairs++;');
  return code;
}
...
http.ServerResponse.prototype.writeHead = function writeHeadV320(statusCode, statusMessage, headers) {
  ...
  if (transformable) this.__ucanV320Chunks = [];
  try {
    this.removeHeader?.('Content-Length');
    this.setHeader?.('X-UCAN-Version', VERSION);
    this.setHeader?.('X-UCAN-Revision', REVISION);
    this.setHeader?.('X-UCAN-Floor-Lock', 'sticky-floor-two');
    if (transformable) {
  ...
  if (nextHeaders && typeof nextHeaders === 'object') {
    ...
    nextHeaders['X-UCAN-Floor-Lock'] = 'sticky-floor-two';
    if (transformable) {
...
http.ServerResponse.prototype.write = function writeV320(chunk, encoding, callback) {
  if (Array.isArray(this.__ucanV320Chunks)) {
    if (chunk != null) this.__ucanV320Chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk), typeof encoding === 'string' ? encoding : 'utf8'));
...
http.ServerResponse.prototype.end = function endV320(chunk, encoding, callback) {
  let body = chunk;
  try {
    if (Array.isArray(this.__ucanV320Chunks)) {
      if (body != null) this.__ucanV320Chunks.push(Buffer.isBuffer(body) ? body : Buffer.from(String(body), typeof encoding === 'string' ? encoding : 'utf8'));
      const combined = Buffer.concat(this.__ucanV320Chunks).toString('utf8');
      delete this.__ucanV320Chunks;
      const pathname = requestPath(this);
      const transformed = SCRIPT_TARGETS.has(pathname) ? transformScript(pathname, combined) : pathname === '/version' || pathname === '/health' || pathname === '/healthz' ? transformJson(combined) : transformHtml(combined);
      body = Buffer.from(String(transformed), 'utf8');
    }
  } catch (reason) {
    console.error('[UCAN V320 response]', reason);
  }
  return baseEnd.call(this, body, encoding, callback);
};

console.info(`[UCAN ${VERSION} ${REVISION}] Piso 2 estable hasta una bajada intencional (${BUILD}).`);
